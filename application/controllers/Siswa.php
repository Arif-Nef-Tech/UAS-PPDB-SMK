...
class Siswa extends CI_Controller
{
    ...
    function lengkapidata()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->session->set_flashdata('pesan', '<strong>Data berhasil disimpan.</strong>');
    ...
    function cetakbuktidaftar($status = null)
    {
        ...
        $pdf->Cell(5, 4, ':', 0, 'J');
        $pdf->MultiCell(125, 4, $siswa['alamat_jalan'] . ' RT ' . $siswa['rt'] . ' RW ' . $siswa['rw'] . ' Kelurahan ' . $siswa['kelurahan'] . ' Kecamatan ' . $siswa['kecamatan'], 0, 'J');
        $pdf->Ln(1);
        $pdf->Cell(10, 4, '');
        ...
        $pdf->Image('./uploads/foto/' . $foto, 115, 223, 20, 30);
    ...
    function kirimbuktidaftar()
    {
        ...
        if ($content == 1) {
            ...
            if ($this->email->send()) {
                ...
                $this->session->set_flashdata('pesan', '<strong>Email sudah dikirim</strong> silahkan check pesan masuk.');
                $this->session->set_flashdata('jenis', 'alert-success');
                redirect('siswa', 'refresh');
            } else {
                $this->session->set_flashdata('pesan', '<strong>Ops..</strong> Email gagal dikirim.');
                $this->session->set_flashdata('jenis', 'alert-danger');
                redirect('siswa', 'refresh');
            }
        } else {
    ...
    function daftarulang()
    {
        ...
        $data['siswa'] = $this->db->get_where('tbl_registrasi', ['nisn' => $this->session->userdata('nisn')])->row_array();
        if ($data['siswa']['seleksi'] != '1' || $data['siswa']['jur'] == '') {
            redirect('', 'refresh');
        }
